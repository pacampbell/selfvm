...
class IRInstr(object):
    ...
    def patch(self, table):
        ...
        # Lets patch up the instruction so its valid; For now we assume jump label
        # TODO: Handle patching of other instructions
        address = table[self._meta['label']]
        self.bincode = (self.bincode & ~0xffffffff) | address
...
def create4(mapping, arguments):
    ...
    imm = 0
    # TODO: Default values
    size = 0b11 # Default to four bytes
    encoding = 0b0001 # Default to two's complement
    needs_patch = False
    label = None
    ...
    for (reg,field) in zip(registers, mapping['args']):
        ...
        if field == 'RD':
            ...
            imm = int(reg)
        elif field == 'LABEL|IMM':
            try:
                imm = int(reg, 0)
            except ValueError as ex:
                imm = 0x0
                needs_patch = True # The value may be a label
                label = reg
        else:
            print('Invalid field \'{}\'for a 4-byte encoding.'.format(field))
    # Build the final instruction
    instruction = (opcode << 16) | (0xf & rd) << 12 | (0xf & rs) << 8 | (0xf & size) << 4 | (0xf & encoding)
    # Now make space for the value
    instruction = (instruction << 32) | imm
    # Print the instruction hex value
    # print('0x{:08x}'.format(instruction))
    return IRInstr(bincode=instruction, size=7, needs_patch=needs_patch, label=label)
...
def main(args):
    with open(args.source_file, 'r') as source_file, open(args.instruction_set, 'r') as is_set:
        ...
        print(lookup_table)
        # TODO: Begin second pass over intermediate code and patch labels
        for instr in ir_list:
            if instr.needs_patch():
                instr.patch(lookup_table)
    with open(args.output_file, 'wb') as output_file:
        # Generate code in place in .bin file
        for instr in ir_list:
            output_file.write(instr.bincode.to_bytes(instr.size, byteorder='little'))
...
if __name__ == "__main__":
    ...
    parser.add_argument('-o', dest='output_file', type=str, default='a.bin', help=help_output)
